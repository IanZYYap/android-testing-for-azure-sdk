 ...
 */
public final class DefaultXmlReader_Android_Reflection extends XmlReader {
    private static final XMLInputFactory XML_INPUT_FACTORY;

    static {
        XMLInputFactory xml_input_factory = null;
        try {
            Method method = XMLInputFactory.class.getMethod("newFactory");
            method.invoke(xml_input_factory);
        } catch (NoSuchMethodException | InvocationTargetException |
                 IllegalAccessException | FactoryConfigurationError e) {
            xml_input_factory = XMLInputFactory.newInstance();
        }
        XML_INPUT_FACTORY = xml_input_factory;
    ...
    public static XmlReader fromStream(InputStream xml) throws XMLStreamException {
        Objects.requireNonNull(xml, "'xml' cannot be null.");
        return new DefaultXmlReader_Android_Reflection(XML_INPUT_FACTORY.createXMLStreamReader(xml));
    }
    ...
    private DefaultXmlReader_Android_Reflection(XMLStreamReader reader) {
        this.reader = reader;
