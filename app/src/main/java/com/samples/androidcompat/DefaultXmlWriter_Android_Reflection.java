 ...
 */
public final class DefaultXmlWriter_Android_Reflection extends XmlWriter {
    private static final XMLOutputFactory XML_OUTPUT_FACTORY;

    static {
        XMLOutputFactory xml_output_factory = null;
        try {
            Method method = XMLOutputFactory.class.getMethod("newFactory");
            method.invoke(xml_output_factory);
        } catch (NoSuchMethodException | InvocationTargetException |
                 IllegalAccessException | FactoryConfigurationError e) {
            xml_output_factory = XMLOutputFactory.newInstance();
        }
        XML_OUTPUT_FACTORY = xml_output_factory;
    }
    ...
    public static XmlWriter toStream(OutputStream xml) throws XMLStreamException {
        ...
        return new DefaultXmlWriter_Android_Reflection(XML_OUTPUT_FACTORY.createXMLStreamWriter(
    ...
    public static XmlWriter toWriter(Writer xml) throws XMLStreamException {
        Objects.requireNonNull(xml, "'xml' cannot be null.");
        return new DefaultXmlWriter_Android_Reflection(XML_OUTPUT_FACTORY.createXMLStreamWriter(xml));
    }

    private DefaultXmlWriter_Android_Reflection(XMLStreamWriter writer) {
        this.writer = writer;
    }
